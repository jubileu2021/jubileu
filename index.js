...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Adeus Arrombado @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				leveloff: ' ❬ X ❭  *desabilitar leveling*',
				levelnoton: '❬ X ❭ *leveling not aktif*',
				levelnol: '*LEVEL FRACO AINDA* 0 °-°',
				error: {
			...
			const isOwner = ownerNumber.includes(sender)
                        ...
                        // Options
                        const botName = 'BOT_Mendigo'
                        const ownerName = 'MENDIGO'
                ...
                let found = false
                    for (let lmt of _limit) {
                        if (lmt.id === sender) {
                            ...
                            if (limitCounts <= 0) return client.sendMessage(from,`Limite seu pedido acabou\n\n_Nota : Limite sera reiniciado a cada jam 21:00!_`, text,{ quoted: mek})
           ...
           return false
			...
			switch(command) {
			                ...
			                const uangku = checkATMuser(sender)
                                        ...
                                        break
                                case '18+menu Nao Entra Seus Kid':
                                        if (!isRegister) return reply(mess.only.daftarB)
		...
		client.sendMessage(from, bahasa(prefix, sender), text, {quoted: mek})
               ...
               break
				case 'demote':
					...
					break
                                case 'randomhentai Proibido para Kids':
                  ...
                  case 'promote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedido Recebido, adicionado posicao como administrador :\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Pedido recebido, adicionado posicao como administrador : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
  ...
  client.updatePresence(from, Presence.composing) 
              ...
              await client.sendMessage(from, options, text)
               break
				...
				case 'marvellogo':
					...
					if (args.length < 1) return reply(`Enviar comando ${prefix}marvellogo teks, contoh ${prefix}marvellogo Lorde Canss`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
		...
		case 'infonomor':
               client.updatePresence(from, Presence.composing) 
				...
				case 'ytmp3':
					...
					break
				case 'ytmp4':
					if (args.length < 1) return reply('Cade o link do video ?')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
				...
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('O que voce quer escrever??')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					teks = body.slice(7)
					reply(mess.wait)
	...
	case 'kbbi':
            client.updatePresence(from, Presence.composing) 
                if (args.length < 1) return reply(`Introduzir Pergunta\nExemplo : ${prefix}kbbi asu`)
                if (!isRegister) return reply(mess.only.daftarB)
                if (isLimit(sender)) return reply(ind.limitend(pusname))
	      tels = body.slice(6)
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
                ...
                break
				case 'blocklist':
					teks = 'Esta e a lista de numeros bloqueados :\n'
                   ...
                   case 'chatlist':
					client.updatePresence(from, Presence.composing)  
					teks = 'Esta e a lista de numeros de bate-papo :\n'
				...
				case 'neonime':
                  ...
                  if (args.length < 1) return reply(`Entrada Texto\nExemplo : ${prefix}Caliph Bot`)
				...
				case 'tts':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'Qual o codigo da linguagem, tio?', text, {quoted: mek})
                                   if (!isRegister) return reply(mess.only.daftarB)
                                   if (isLimit(sender)) return reply(ind.limitend(pusname))
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Cade o texto tio', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				case 'tagall':
					...
					mentions('╔══✪〘 Chamando Os Cornos Do Grupo 〙✪══\n╠➥'+teks+'╚═〘 - - - - 〙', members_id, true)
					break
                case 'tagall2':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 Lista de Gados 〙✪══\n╠➥'+teks+'╚═〘 - - - - 〙', text, {quoted: mek})
					break
                case 'tagall3':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 Chamando os cornos 〙✪══\n╠➥'+teks+'╚═〘 - - - - - 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
					...
					client.sendMessage(from, '╔══✪〘 chamando todos boi 〙✪══\n╠➥'+teks+'╚═〘 - - - - - 〙', text, {quoted: mek})
					break
                case 'tagall5':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						reply('Grupo de transmissao de sucesso')
					}
					break
				case 'alay':
                    client.updatePresence(from, Presence.composing) 
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'Adeus corno 👋', text) // ur cods
					}, 0)
                     break

				case 'chord':
					if (args.length < 1) return reply('onde esta o titulo da musica')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)					
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           break
				...
				case 'kick':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'Pedidos recebidos, removi o puto ou a vadia:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`Pedidos recebidos, emitidos , removi o vagabundo ou a vadia  : @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
				...
				case 'ocr':
					...
					break
                     ...
                     const bug = body.slice(5)
                      if (pesan.length > 300) return client.sendMessage(from, 'Desculpe, texto muito longo, Maximo de 300 textos', msgType.text, {quoted: mek})
                    ...
                    reply('O problema foi relatado ao proprietario do BOT, relatorios falsos/main2 nao sera respondido.')
                    break
               case 'apakah':
               client.updatePresence(from, Presence.composing) 
	    ...
	    case 'kapankah':
			...
			case 'closegc':
					...
					var nomor = mek.participant
					const close = {
					text: `Grupo fechado pelo administrador @${numero.split("@s.whatsapp.net")[0]}\nagora *apenas administrador* quem pode enviar mensagens`,
					contextInfo: { mentionedJid: [nomor] }
					}
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`❌ Falhou, na hora da conversao ${tipe} ke stiker`)
							})
							.on('end', function () {
								console.log('Finish')
				...
				case 'toimg':
				    client.updatePresence(from, Presence.composing)
                                    if (!isRegister) return reply(mess.only.daftarB)
					if (!isQuotedSticker) return reply('❌ responda com figurinha normal ❌')
					reply(mess.wait)
                ...
                case 'ninjalogo':
                      if (args.length < 1) return reply('Cade o texto Bicho burro?')
                      if (!isRegister) return reply(mess.only.daftarB)
                      if (isLimit(sender)) return reply(ind.limitend(pusname))
                      gh = body.slice(11)
                      gl1 = gh.split("|")[0];
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					if (args.length < 1) return reply(mess.blank)
                                        ...
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)
					if (tels.length > 15) return reply('O texto e muito longo, maximo de 20 caracteres')
					reply(mess.wait)
				...
				case 'firetext':
					if (args.length < 1) return reply(mess.blank)
                                        ...
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					tels = body.slice(7)
					if (tels.ength > 10) return reply('O texto e muito longo, ate 9 caracteres')
					reply(mess.wait)
				...
				if (isRegister) return reply(mess.only.daftarB)
					...
					client.sendMessage(from, 'Pergunte : *'+watak+'*\n\nResponda : '+ tak, text, { quoted: mek })
                                        await limitAdd(sender)
				    break
				case 'hobby':
				if (isRegister) return reply(mess.only.daftarB)
					...
					break
                                case 'nsfwneko':
				    try{
						if (!isNsfw) return reply('❌ *NSFW MORTO* ❌')
                                                if (!isRegister) return reply(mess.only.daftarB)
                                                if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				case 'logowolf':
					...
					if (args.length < 1) return reply(`onde esta o texto? contoh ${prefix}logowolf Nazwa|Canss`)
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
					reply(mess.wait)
            ...
            case 'leveling':
                ...
                if (args[0] === '1') {
                    ...
                    reply(' *Digite o comando 1 para ativar, 0 desabilitar* \n *Exemplo: ${prefix}leveling 1*')
                }
            break
                                case 'infogempa':
			       ...
			       reply(anu.result.jadwalTV)
					break*/
                                ...
                                case 'shadow':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde esta o texto hum\nExemplo: ${prefix}shadow NazwaCanss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'coffee':
                                        var gh = body.slice(7)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde esta o texto hum\nExemplo: ${prefix}coffee NazwaCanss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
                                ...
                                case 'lovepaper':
                                        var gh = body.slice(10)
                                        var teks1 = gh.split("|")[0];
                                        if (args.length < 1) return reply('onde esta o texto hum\nExemplo: ${prefix}lovepaper mendigoCanss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                        reply(mess.wait)
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Sucesso ative o modo simi neste grupo ✔️')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isOwner) return reply(mess.only.ownerB)
					if (args.length < 1) return reply('Marque alvo que voc� deseja clonar')
					...
					try {
						...
						mentions(`Foto de perfil Bem-sucedida di perbarui menggunakan foto profile @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply('Gagal om')
					}
					break
                                case 'pubglogo':
                                        ...
                                        buffer = await getBuffer(anu.result)
                                        client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Aqui esta o logotipo, mana...'})
                                        await limitAdd(sender)
                                        break
                                case 'herrypotter':
                                case 'harrypotter':
                                        var gh = body.slice(12)
                                        if (args.length < 1) return reply('onde esta o texto hum\nExemplo: ${prefix}harrypotter TiocafeCanss')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (isLimit(sender)) return reply(ind.limitend(pusname))
                                ...
                                case 'bal':
                                        ...
                                        break
                                case 'buylimit':
                                        if (args.length < 1) return reply('Qual o limite que voce quer comprar mano? Certifique-se de ter dinheiro suficiente, mana! \n\nComo verificar dinheiro: ${prefix}bal')
                                        if (!isRegister) return reply(mess.only.daftarB)
                                ...
                                case 'event':
                                        ...
                                        if (Number(args[0]) === 1) {
                                                ...
                                                reply('*❬ SUCSESSO ❭ Ative EVENTO neste grupo*')
                                        } else if (Number(args[0]) === 0) {
                                                event.splice(from, 1)
                                                fs.writeFileSync('./database/json/event.json', JSON.stringify(event))
					...
					break*/
                                case 'translate':
                                        ...
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        if (args.length < 1) return reply(`onde esta o codigo do idioma sis?\nExemplo: ${prefix}translate en|Hai, aku Nazwa`)
                                ...
                                case 'tagme':
                                        if (!isRegister) return reply(mess.only.daftarB)
                                        const tagme = {
                                                text: `@${nom.split("@s.whatsapp.net")[0]} Isso por tag!`,
                                                contextInfo: { mentionedJid: [nom] }
                                        }
